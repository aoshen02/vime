...
This path is **not yet supported by Vime's pinned vLLM**. The Megatron-side
alignment hooks are present, but the required rollout-side sparse MLA,
DeepGEMM and DeepEP numerical contracts are not fully validated.

`tests/test_glm52_6layer_deterministic_e2e.py` and
`tests/test_glm52_layerwise_zero_e2e.py` currently raise an explicit unsupported
error; they are not passing regression gates. Slime's reported log-prob
difference below 1e-6 and exact layerwise equality are upstream reference
targets, not Vime results.

Track the missing engine behavior and corresponding vLLM PRs in the
