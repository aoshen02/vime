...
directory; Vime invokes a Ray actor on every rollout host to apply and verify it, then the engine
reloads the patched local checkpoint through the ordinary `update_weights_from_disk` endpoint.
This host-local path supports Vime-launched engines in the Ray cluster; external vLLM engines
must use full disk reload directly from the shared directory.
...
| `--update-weight-local-checkpoint-dir` | Host-local (e.g. NVMe) full HF checkpoint kept in sync by each rollout-host actor — deltas are applied into it in place; a published full checkpoint replaces it. Each host seeds it from the engine's model path on the first pull. |
...
1. **Seed.** On the first sync the trainer captures a CPU snapshot of every parameter — seeded
   ...
   The trainer also issues `pull_weights(target_version=0)` so every host materializes
...
3. **Pull.** The trainer calls `pull_weights` on every rollout-host Ray actor. Each host applies
   the new version's delta into its local checkpoint in place under a per-host file lock.
...
propagates through the Ray call, so a corrupt delta or a wrong base fails loud
...
- `--vllm-custom-pull-weights-pre-read-hook` (rollout-host side): called on each host before its
  Ray actor reads the delta directory (e.g. refresh the mount's view).
