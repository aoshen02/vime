...
export VLLM_VERSION="v0.5.15.post1"
export VLLM_COMMIT="0b3bb0cbe31873994c9f989fddfe2f87ca839fdd"
...
# install vllm. The Dockerfile starts FROM slimerl/vllm:v0.5.15.post1-cu129
...
pip install --force-reinstall --no-deps \
  vllm-kernel==0.4.4 sgl-deep-gemm==0.1.4 \
...
# The VLLM dependency set includes FA4, while the validated TE 2.16 CP stack
...
pip install https://github.com/zhuzilin/sgl-router/releases/download/v0.3.2-9daabcd/vllm_router-0.3.2-cp38-abi3-manylinux_2_28_x86_64.whl --force-reinstall
python -c "import vllm_router; assert 'slime' in vllm_router.__version__"
...
cd $SLIME_DIR/vime/backends/megatron_utils/kernels/int4_qat
...
cd $BASE_DIR/vllm
for patch_name in vllm.patch vllm-top_p.patch vllm-release_hicache.patch vllm-pull_weights.patch; do
