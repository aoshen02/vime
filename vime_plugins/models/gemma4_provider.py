...
from vime_plugins.models.gemma4 import _load_hf_text_config, get_gemma4_attention_layout
...
def _install_hooks(model, args, config, pre_process, post_process):
    ...
    if hasattr(inner, "rotary_pos_emb"):
        ...
        global_head_dim = get_gemma4_attention_layout(hf_text).global_head_dim
...
def _read_layer_scalars_from_safetensors(hf_checkpoint: str) -> dict[int, float] | None:
    ...
    no safetensors weights or no layer_scalar weights. Only called on rank 0 -
    ...
    if os.path.exists(index_path):
        ...
        model_path = os.path.join(hf_checkpoint, "model.safetensors")
        if not os.path.exists(model_path):
            logger.warning("No safetensors checkpoint at %s", hf_checkpoint)
            return None
    ...
    if not os.path.exists(index_path):
        with safe_open(model_path, framework="pt", device="cpu") as sf:
            weight_map = {key: "model.safetensors" for key in sf.keys()}

    scalar_keys_by_file: dict[str, list[str]] = {}
    for key, filename in weight_map.items():
        if "layer_scalar" in key:
            scalar_keys_by_file.setdefault(filename, []).append(key)

    scalars: dict[int, float] = {}
    for filename, keys in scalar_keys_by_file.items():
