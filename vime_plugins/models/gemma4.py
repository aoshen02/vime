...
class Gemma4SelfAttention(SelfAttention):
    ...
    def _split_qkv_global_k_eq_v(self, hidden_states):
        ...
        if self.config.num_query_groups < self.world_size:
            ...
            group_rank = rank // (self.world_size // self.config.num_query_groups)
            group_width = mixed_qkv.size(-1) // self.config.num_query_groups
            mixed_qkv = mixed_qkv[..., group_rank * group_width : (group_rank + 1) * group_width]
        ...
        if self.config.num_query_groups < self.world_size:
            ...
            replica_rank = rank % replicas
            query_heads_per_rank = self.num_attention_heads_per_partition // replicas
            query = query[:, :, replica_rank * query_heads_per_rank : (replica_rank + 1) * query_heads_per_rank, :]
...
class Gemma4AttentionLayout:
    layer_types: tuple[str, ...]
    global_attn_layers: frozenset[int]
    local_head_dim: int
    global_head_dim: int
...
def get_gemma4_attention_layout(hf_text) -> Gemma4AttentionLayout:
    layer_types = tuple(getattr(hf_text, "layer_types", ()))
    per_layer_config = getattr(hf_text, "per_layer_config", None)

    if per_layer_config is not None and layer_types:
        ...
        for layer_idx, layer_type in enumerate(layer_types):
            ...
            if layer_type in representative_geometry and geometry != representative_geometry[layer_type]:
                raise ValueError(f"Gemma4 {layer_type} layers have inconsistent attention geometry")
            representative_configs.setdefault(layer_type, layer_config)
            representative_geometry.setdefault(layer_type, geometry)

        local_config = next(
            (config for layer_type, config in representative_configs.items() if layer_type != "full_attention"),
            per_layer_config[0],
        )
        global_config = representative_configs.get("full_attention", local_config)
        ...
        sliding_window = local_config.sliding_window
    else:
        local_head_dim = hf_text.head_dim
        global_head_dim = getattr(hf_text, "global_head_dim", local_head_dim)
        local_num_kv_heads = hf_text.num_key_value_heads
        global_num_kv_heads = getattr(hf_text, "num_global_key_value_heads", local_num_kv_heads)
        sliding_window = hf_text.sliding_window

    return Gemma4AttentionLayout(
        layer_types=layer_types,
        global_attn_layers=frozenset(i for i, layer_type in enumerate(layer_types) if layer_type == "full_attention"),
        local_head_dim=int(local_head_dim),
        global_head_dim=int(global_head_dim),
        local_num_kv_heads=int(local_num_kv_heads),
...
def _apply_core_config(config, hf_text):
    ...
    attention_layout = get_gemma4_attention_layout(hf_text)
    config.__class__ = Gemma4TransformerConfig
    config.global_kv_channels = attention_layout.global_head_dim
    config.global_num_query_groups = attention_layout.global_num_kv_heads
