...
from vime_plugins.models.gemma4 import get_gemma4_attention_layout
...
class Gemma4Bridge(Gemma3Bridge):
    ...
    def _attention_shape_for_hf_weights(self, hf_weights: list[torch.Tensor]) -> tuple[int, int]:
        ...
        if len(hf_weights) == 2:
            return attention_layout.global_num_kv_heads, attention_layout.global_head_dim
        if len(hf_weights) == 3:
            return attention_layout.local_num_kv_heads, attention_layout.local_head_dim
    ...
    def _build_config(self):
        ...
        base_kwargs = dict(
            ...
            num_query_groups=attention_layout.local_num_kv_heads,
            kv_channels=attention_layout.local_head_dim,
        )
        ...
        supported_per_layer_attributes = {"head_dim", "num_key_value_heads"}
        per_layer_attributes = set(getattr(hf_text, "per_layer_attributes", ()) or ())
        unsupported_per_layer_attributes = per_layer_attributes - supported_per_layer_attributes
        if unsupported_per_layer_attributes:
            raise ValueError(
                "Gemma4Bridge does not support per-layer attributes: " f"{sorted(unsupported_per_layer_attributes)}"
            )

        allow_attribute = "allow_global_per_layer_attribute_access"
        previous_allow_global = getattr(hf_text, allow_attribute, False)
        setattr(hf_text, allow_attribute, True)
        try:
            return self._build_base_config(**base_kwargs)
        finally:
            setattr(hf_text, allow_attribute, previous_allow_global)
