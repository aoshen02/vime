...
def test_ordered_ep_gather_backward_matches_weighted_route_sum(monkeypatch):
    ...
    output = deepgemm_moe_forward._VLLMEPGatherWithBF16Backward.apply(
        hidden,
        indices,
        weights,
        output_index,
...
def test_deepep_alignment_is_disabled_outside_deterministic_mode(monkeypatch):
    ...
    deepgemm_moe_forward.enable_vllm_deepep_moe_alignment(args, object(), "")
...
def test_deepep_alignment_captures_ordered_topk_without_r3(monkeypatch):
    ...
    assert deepgemm_moe_forward._patch_vllm_deepep_layer(mlp, global_layer=3)
