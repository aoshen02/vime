...
def install_megatron_stubs() -> None:
    ...
    _ensure_module("megatron.core.tensor_parallel")
    mappings = _ensure_module("megatron.core.tensor_parallel.mappings")
    mappings.all_gather_last_dim_from_tensor_parallel_region = lambda tensor: tensor
...
def install_mbridge_stubs() -> None:
    ...
    core.registered_models = []

    def register_model(model_types):
        core.registered_models.extend(model_types)
        return lambda cls: cls

    core.register_model = register_model
...
def _legacy_gemma4_attention_layout(hf_text):
    ...
    return types.SimpleNamespace(
        ...
        global_head_dim=getattr(hf_text, "global_head_dim", hf_text.head_dim),
        local_num_kv_heads=hf_text.num_key_value_heads,
        global_num_kv_heads=getattr(hf_text, "num_global_key_value_heads", hf_text.num_key_value_heads),
        sliding_window=getattr(hf_text, "sliding_window", 0),
    )
...
def load_gemma4_provider_module():
    ...
    gemma4_stub.get_gemma4_attention_layout = _legacy_gemma4_attention_layout
