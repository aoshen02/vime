...
def test_gemma4_bridge_registers_hf_wrapper_model_types():
    load_gemma4_bridge_class()

    from mbridge import core

    assert "gemma4_unified" in core.registered_models
...
def test_gemma4_bridge_dense_config_does_not_set_moe_kwargs():
    ...
    assert cfg["num_query_groups"] == CFG_31B.num_key_value_heads
    assert cfg["kv_channels"] == CFG_31B.head_dim
    assert "num_moe_experts" not in cfg
    assert "moe_router_topk" not in cfg
    assert "moe_ffn_hidden_size" not in cfg


def test_gemma4_bridge_rejects_unsupported_per_layer_attributes():
    local_config = SimpleNamespace(head_dim=256, num_key_value_heads=16, sliding_window=1024)
    global_config = SimpleNamespace(head_dim=512, num_key_value_heads=4, sliding_window=1024)
    bridge = object.__new__(load_gemma4_bridge_class())
    hf_config = vars(CFG_31B).copy()
    hf_config.update(
        per_layer_config=[local_config, global_config],
        per_layer_attributes={"head_dim", "num_key_value_heads", "intermediate_size"},
