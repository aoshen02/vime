...
def test_read_layer_scalars_supports_single_safetensors_file(tmp_path):
    scalars = {0: 0.5, 1: 1.5, 2: 2.5}
    _write_fake_single_safetensors_layer_scalars(tmp_path, scalars)

    assert _provider._read_layer_scalars_from_safetensors(str(tmp_path)) == pytest.approx(scalars)
...
def test_load_layer_scalars_skips_when_no_index_file_with_opt_in(tmp_path, monkeypatch, caplog):
    ...
    assert any("No safetensors checkpoint" in r.message for r in caplog.records)
