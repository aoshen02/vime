...
def test_pipeline_stage_input_rmsnorm_uses_one_vllm_forward_and_analytic_backward(
    monkeypatch,
):
...
def test_pipeline_stage_input_rmsnorm_is_opt_in(monkeypatch):
    model = torch.nn.Module()
    monkeypatch.delenv("MEGATRON_USE_VLLM_FUSED_RESIDUAL_RMS", raising=False)
...
def test_absorbed_kv_rmsnorm_uses_one_vllm_forward_and_analytic_backward(
    ...
    deepgemm_forward.enable_vllm_absorbed_kv_rmsnorm(args, model, "")
    input_ = torch.tensor([[5.0, 7.0]], requires_grad=True)
    expected_input = input_.detach().clone().requires_grad_()
    expected_weight = (
...
def test_final_rmsnorm_uses_exact_fp32_residual_and_analytic_backward(monkeypatch):
    ...
    model.decoder.layers[-1]._vllm_residual_sum_fp32 = torch.tensor([[3.0, 4.0]])
    monkeypatch.setenv("MEGATRON_USE_VLLM_FUSED_RESIDUAL_RMS", "1")

    deepgemm_forward.enable_vllm_final_rmsnorm(None, model, "")
    input_ = torch.tensor([[5.0, 7.0]], requires_grad=True)
    expected_input = input_.detach().clone().requires_grad_()
...
def test_vllm_router_value_allows_saved_rollout_replay_without_live_vllm():
    ...
    deepgemm_forward.enable_vllm_router_gemm(args, [], store_prefix="")


def test_vllm_swiglu_uses_one_forward_and_analytic_backward(monkeypatch):
...
def test_vllm_swiglu_mlp_accepts_current_megatron_padding_mask(monkeypatch):
    ...
    monkeypatch.setattr(
        deepgemm_forward,
        "_vllm_swiglu_with_megatron_backward",
        lambda value: value[..., :2],
    )

    assert deepgemm_forward._wrap_vllm_swiglu_mlp(
        mlp,
