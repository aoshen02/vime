...
Drives a full Megatron -> VLLM online-weight-update rollout of a 6-layer
...
lack the deterministic VLLM / DeepGEMM / DeepEP stack. Model and prompt
...
assert "enable_fp32_moe_router" in ServerArgs.__dataclass_fields__, "VLLM lacks enable_fp32_moe_router"
"""
...
def _deterministic_env(
    ...
    env.update(
        {
            "PYTHONPATH": _pythonpath(vllm_root, megatron_root),
            "PYTHONUNBUFFERED": "1",
            "NO_PROXY": "*",
            "no_proxy": "*",
        }
...
def _skip_reason(vllm_root, megatron_root) -> str | None:
    ...
    probe_env = {**os.environ, "PYTHONPATH": _pythonpath(vllm_root, megatron_root)}
...
def _train_args(
    ...
    *,
    ...
    groups = [
        ...
        # VLLM rollout (deterministic DSA + DeepEP low-latency + DeepGEMM)
        f"--rollout-num-gpus-per-engine {NUM_GPUS} --vllm-server-concurrency 128 "
        "--vllm-mem-fraction-static 0.70 --vllm-enable-dp-attention --vllm-enable-dp-lm-head "
        f"--vllm-ep-size {NUM_GPUS} --vllm-dp-size {NUM_GPUS} --vllm-moe-dp-size 1 "
        "--vllm-moe-dense-tp-size 1 --vllm-moe-a2a-backend deepep --vllm-deepep-mode low_latency "
        "--vllm-moe-runner-backend deep_gemm --vllm-fp8-gemm-runner-backend deep_gemm "
        f"--vllm-page-size 64 --vllm-kv-cache-dtype {kv_cache_dtype} --vllm-attention-backend dsa "
        "--vllm-dsa-prefill-backend flashmla_sparse --vllm-dsa-decode-backend flashmla_sparse "
        "--vllm-dsa-topk-backend torch --vllm-chunked-prefill-size 4096 --vllm-context-length 8192 "
        "--vllm-max-prefill-tokens 4096 --vllm-enable-fp32-moe-router "
        "--vllm-enable-deterministic-inference --vllm-disable-prefill-cuda-graph "
        "--vllm-cuda-graph-max-bs-decode 64 --vllm-watchdog-timeout 7200 --vllm-dist-timeout 1800 "
    ...
    if vllm_layerwise_dump is not None:
        groups.append(
            f"--vllm-debug-tensor-dump-output-folder {vllm_layerwise_dump} "
            "--vllm-debug-tensor-dump-layers 0 1 2 3 4 5"
        )
    return " ".join(groups)
...
def run_gate(*, layerwise_zero: bool = False, rollout_max_response_len: int = 4096) -> None:
    ...
    kv_cache_dtype = os.environ.get("VLLM_KV_CACHE_DTYPE", "fp8_e4m3")
    if kv_cache_dtype not in {"bfloat16", "fp8_e4m3"}:
        raise ValueError("VLLM_KV_CACHE_DTYPE must be bfloat16 or fp8_e4m3, " f"got {kv_cache_dtype!r}")

    reason = _skip_reason(vllm_root, megatron_root)
    ...
    # (VLLM engine + Megatron train actor) inherits the exact numerical stack,
    # matching how the standalone gate launches.
    env = {
        **os.environ,
        **_deterministic_env(vllm_root, megatron_root, kv_cache_dtype),
    }
    # vllm-router's Rust worker discovery does not currently honor
    ...
    # colocated Ray/VLLM endpoints, so keep its whole child process tree
    ...
    with tempfile.TemporaryDirectory(prefix="glm52_6layer_gate_") as tmp:
        ...
        vllm_layerwise_dump = os.path.join(tmp, "vllm_layerwise")
        if layerwise_zero:
            env.update(
                {
        ...
        argv = _train_args(
            ...
            vllm_layerwise_dump=(vllm_layerwise_dump if layerwise_zero else None),
        ).split()

        _run(["pkill", "-9", "vllm"], check=False)
...
def test_glm52_alignment_gate_trains_all_main_model_parameters_without_r3():
    ...
    assert argv[argv.index("--vllm-kv-cache-dtype") + 1] == "fp8_e4m3"
