"""Host-local pull of published weights for Vime-managed vLLM nodes.
...
``pull()`` is safe to call concurrently on a host: a per-host file lock serializes
the work and an applied-version marker makes extra calls no-ops.
"""
...
def _reset_checkpoint(src_dir: str, local_checkpoint_dir: str, version: int) -> None:
    ...
    logger.info("Pulling full checkpoint v%d %s -> %s", version, src_dir, local_checkpoint_dir)
    ...
    for entry in src_files:
        ...
        if copied != entry.stat().st_size:
            raise RuntimeError(f"size mismatch copying {entry.name}: src {entry.stat().st_size} != local {copied}")
